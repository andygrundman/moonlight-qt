...
//
// This class relies on thread ownership rather than full internal locking:
//
// - init() must be called once before any concurrent use
// - observeFramePts() must be called only from the decoder thread
// - decideAdvanceCount() must be called only from the render thread
// - setDisplayHz() may be called from the render thread/pacer thread
// - cross-thread communication happens only through atomic published values

class FrameCadence
{
  public:
    // Singleton
    static FrameCadence& instance();
    ...
    // Returns the pts of the previous frame.
    int64_t observeFramePts(int64_t pts90k);
  ...
  private:
    FrameCadence();
	  FrameCadence(const FrameCadence&) = delete;
	  FrameCadence& operator=(const FrameCadence&) = delete;

    // Decoder-thread owned state.
    int64_t m_lastPts90k = 0;

    double m_streamPeriodEwmaMs = 0.0;
    double m_streamEwmaAlpha = 0.1;
