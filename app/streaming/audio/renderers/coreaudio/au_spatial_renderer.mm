                              ...
                              AudioBufferList * ioData)
{
    ...
    for (uint32_t i = 0; i < ioData->mNumberBuffers; i++) {
        memset((float *)ioData->mBuffers[i].mData, 0, inNumberFrames * 4);
    }
    ...
    for (uint32_t frame = 0; frame < inNumberFrames; frame++) {
        for (uint32_t channel = 0; channel < channelCount; channel++) {
            ...
            uint32_t interleavedOffset = (frame * channelCount) + channel;
            memcpy(channelBuffer + frame, ringBuffer + interleavedOffset, 4);
        }
    }
...
void AUSpatialRenderer::process(AudioBufferList* __nullable outputABL,
                                const AudioTimeStamp* __nullable inTimeStamp,
                                float inNumberFrames)
{
    AudioUnitRenderActionFlags actionFlags = {};
    AudioUnitRender(m_Mixer, &actionFlags, inTimeStamp, 0, inNumberFrames, outputABL);
}
