# This script is the same as generate-dmg.sh with the additional requirement that SIGNING_IDENTITY be
# a paid Apple developer account. This will enable the spatial audio entitlements for head-tracking
# and personalized HRTF.
...
if [ "$SIGNING_IDENTITY" != "" ]; then
  ...
  codesign --force --deep --options runtime --timestamp --entitlements $SOURCE_ROOT/app/spatial-audio.entitlements --sign "$SIGNING_IDENTITY" $BUILD_FOLDER/app/Moonlight.app || fail "Signing failed!"
fi
